...
def load_patches_from_file (file, patch_size, random, n_patches=3, stride=32, cut_size=None, preprocess_limit = 100):
...
def load_patches (folder, patch_size, random=True, n_patches=3, stride=32, cut_size=None, preprocess_limit = 100):
    patches = []
    for file in os.listdir(folder):
        if file.endswith(".bmp") or file.endswith(".tif"):
            ret, img = load_patches_from_file(os.path.join(folder, file), patch_size, random, n_patches, stride, cut_size, preprocess_limit)
            for r in ret:
                patches.append(r)
    return patches
